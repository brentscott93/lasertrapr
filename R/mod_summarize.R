...
mod_summarize_server <- function(input, output, session, f){
  ...
  observeEvent(input$go, {
    ...
    trap_data <- purrr::map(trap_data_paths$path, function(x){ incProgress(0.0025)
     ...
     setProgress(detail = 'rbind')                                
    trap_data <- data.table::rbindlist(trap_data, fill = T)
    setDT(trap_data)                      
  ...
  output$table <- DT::renderDT({
    validate(need('conditions' %in% colnames(data$summary), 'Select completed project, choose options, and click summarize'))
    data$summary %>% 
