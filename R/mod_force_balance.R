#' force_balance UI Function
#' @description A shiny Module.
#' @param id,input,output,session Internal parameters for {shiny}.
#' @importFrom shiny NS tagList
#' @noRd
mod_force_balance_ui <- function(id){
  ns <- NS(id)
  tagList(
    fluidRow(
    ...
    fluidRow(
      column(4,
             ...
             box(width = NULL, title = 'Force Balance Controls',
    ...
    fluidRow(
      ...
      box(width = NULL, title = 'Analyzed Data', collapsible = T, collapsed = T,
        ...
        dygraphs::dygraphOutput(ns('overlay_dygraph_other'), height = '200px'),
...
mod_force_balance_server <- function(input, output, session, f){
 ns <- session$ns
 ...
 hm <- reactiveValues(analyze = 0, use_channels_options = c("Mean/Var", "Variance"))
  ...
  observeEvent(hm$analyze, ignoreInit = T, {
    ...
    purrr::walk(trap_data, ~force_balance_analysis(
                                    trap_data = .x,
                                    f = f,
                                    hz = a$hz,
                                    w_width = a$w_width,
  ...
  output$overlay_dygraph <- dygraphs::renderDygraph({
    ...
    ## if(channels == 1){
      ...
      ## other_raw <- trap_data()$trap$other_processed_bead
    ## } else if(channels == 2){
   ...
   ## pni <-  events$peak_nm_index
      ...
      # } else {
        ...
        overlay_dy <-  dygraphs::dygraph(d, group = "group") |> #raw_data_dygraph
  ...
  output$overlay_dygraph_other <- dygraphs::renderDygraph({
    req(trap_data())
    hz <- trap_data()$options$hz[[1]]
    ## if(channels == 1){
      other_raw <- trap_data()$trap$other_processed_bead
    ...
    periods_df <- data.table::data.table(
                                start = trap_data()$events$other_peak_window_start_dp/hz,
                                stop = trap_data()$events$other_peak_window_stop_dp/hz,
                                keep = trap_data()$events$keep,
                                event_user_excluded = trap_data()$events$event_user_excluded,
                                front_signal_ratio = trap_data()$events$front_signal_ratio,
                                back_signal_ratio = trap_data()$events$back_signal_ratio,
                                color = scales::alpha("#FA8072" , 0.4))


   periods_df <- periods_df[keep == T & event_user_excluded == F]
   periods_df <- periods_df[front_signal_ratio >= ro$front_signal_ratio & back_signal_ratio >= ro$back_signal_ratio]

         dygraphs::dygraph(d, group = "group") |> #raw_data_dygraph
                        dygraphs::dySeries('other_raw', color = 'black') |>
                        add_shades(periods_df) |> #raw_periods
                        dygraphs::dyAxis('x', label = 'seconds', drawGrid = FALSE) |>
                        dygraphs::dyAxis('y', label = 'nm', drawGrid = FALSE)
     # }
  ...
  observeEvent(input$options, {
    ...
    output$numbers <- renderPlot({
      ...
      step <-
        ...
        ##              fill = "grey40",
        ##              binwidth = 2)+
        stat_ecdf(data = measured_events, aes(x = force_balance_pn), pad = FALSE)+
        geom_vline(aes(xintercept = mean(measured_events$force_balance_pn)), linetype = "dashed")+
        geom_label(aes(mean(measured_events$force_balance_pn),
                       y = 1,
                       label = paste0("bar(x)==", round(mean(measured_events$force_balance_pn), 1))),
                   parse = TRUE,
                   size = 6)+
        ggtitle("Force Balance")+
        xlab("piconewtons")+
        ylab('ECDF')+
...
# mod_force_balance_ui("force_balance")

## To be copied in the server
# callModule(mod_force_balance_server, "force_balance_ui")
