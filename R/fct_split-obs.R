  ...
  withProgress(message = 'Uploading trap data', value = 0, max = 1, min = 0, {
    ...
    for(c in seq_along(create_obs)){
      if(c < 10){
        ...
        o <- mutate(o, obs =  paste0("obs-0", c)) %>% dplyr::select(project, conditions, date, obs, everything())
         ...
         data.table::fwrite(o, file = file.path(date$path, paste0("obs-0", c), "options.csv"), sep = ",")
          ...
          o <- mutate(o, obs =  paste0("obs-", c)) %>% dplyr::select(project, conditions, date, obs, everything())
